...
public class ProductDetailOvenService {

public ArrayList<ProAtt> productDetail(int productNo){
		
		Connection conn = getConnection();
		ArrayList<ProAtt> list = new UserProductDao().detailOven(conn,productNo);
		close(conn);
	...
	public UserProduct productDetailFood(int productNo){
		...
		Connection conn = getConnection();
		UserProduct p = new UserProductDao().productDetailOven(conn,productNo);
		close(conn);
		return p;  //상품테이블값
	}
}
