...
<body>
	...
	<p>
		...
		* 동기식 / 비동기식 <br>
		> 동기식 방식 요청 (a, form submit, location.href, redirect을 이용해서 url요청) <br>
		<ul>
			<li>사용자의 요청 처리 후 그에 해당하는 응답페이지가 돌아와야만 그 이후의 작업 가능</li>
			<li>서버에 요청한 결과까지의 시간이 지연되면 무작정 계속 기다려야됨 (흰페이지로 보여질꺼임)</li>
			<li>응답페이지가 전체 로딩되면서 보여짐 (페이지 깜박거림 발생, 스크롤 최상단으로 올라감)</li>
		</ul>
		...
		<ul>
			...
			<li>요청 보내놓고 그에 해당하는 응답(데이터)이 돌아돌때까지 현재 페이지에서 다른 작업을 할 수 있음</li>
			<li>돌아오는 응답데이터로 현재 페이지 일부 영역에 출력시켜줄 수 있음</li>
			<li>페이지 전체가 리로딩 되는게 아니기 때문에 페이지 깜박거리지 않음 (스크롤 올라가지 않음)</li>
		</ul>
		ex) 실시간 급상승 검색어 로딩, 검색어 자동완성, 아이디중복체크, 찜하기/해제하기, 댓글, 추천, 무한스크롤링(페이징대체) 등등, ...
		<br><br>
	...
	* 주요속성
	...
	- complete : ajax통시 성공했든 실패했든 간에 무조건 실행할 함수 정의
	...
	* 부수적인속성
	...
	<script>
		function test1(){
			...
			$.ajax({
				...
				success:function(result){ // 매개변수에 요청처리후에 돌아오는 응답데이터 담길꺼임
	...
	검색하고자 하는 회원아이디 : <input type="text" id="input3">
	...
	<script>
		function test4(){
			$.ajax({
				url:"/web/test4.do",
				success:function(result){
					console.log(result);// [{key:value, ..}, {key:value, ..}, {key:value, ..}]
